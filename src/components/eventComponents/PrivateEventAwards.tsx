...
import { AwardDraft } from "@/src/lib/global_types";

const PrivateEventAwards = ({ awards }: { awards: AwardDraft[] }) => {
  return (
      ...
      <div className="flex flex-col w-full max-w-6xl px-5 sm:px-10 md:px-18 py-10 md:py-14 gap-6 md:gap-8">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-white/70 text-sm leading-relaxed max-w-3xl">
            Awards help teams understand what excellence looks like—and what
        ...
        {awards.length === 0 ? (
          <div className="w-full rounded-3xl bg-white/4 border border-white/10 px-6 py-6 md:px-8 md:py-8">
            <div className="text-white/80 font-semibold">No awards yet</div>
            <div className="text-white/60 text-sm leading-relaxed">
              Awards will appear here once they’re added in event settings.
            </div>
          </div>
        ) : (
