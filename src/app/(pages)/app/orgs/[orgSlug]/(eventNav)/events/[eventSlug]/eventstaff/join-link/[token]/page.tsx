...
import {
  acceptEventStaffInviteLink,
  fetchEventStaffJoinLinkPageData,
} from "@/src/actions/event_staff_invites_actions";
import { baseUrl } from "@/src/lib/utils";

const EventStaffLinkInvitePage = async ({
  params,
}: {
      ...
      <div className="relative flex items-center justify-center px-5 py-10">
        <JoinGate
          ...
          entity={{
            ...
            description: null,
            role: data.link?.role ?? null,
          }}
          session={data.session}
          isMember={data.isStaff}
          token={token}
          disabledReason={disabledReason}
          acceptAction={acceptEventStaffInviteLink}
        />
      </div>
    </div>
  );
...
export default EventStaffLinkInvitePage;
